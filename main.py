...
path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".cache-" + username) # look in cache_files
token = util.prompt_for_user_token(username, scope, client_id, client_secret, redirect_uri, path)     #get token
...
def emailMessages(message_dict):
    ...
    with smtplib.SMTP(smtp_server, port) as server:
        ...
        for receiver_email in message_dict.keys():
            ...
            try:
                server.sendmail(sender_email, receiver_email, message.encode('utf-8'))      #send email with utf-8 encoded message to account for special characters / emojis
                print("SUCCESS: email sent!", file=sys.stderr)
            except:
                print('FAILURE: email failed to send', file=sys.stderr)
...
def main():
    if datetime.datetime.today().weekday() == 0:                # only updating on Sunday. PythonAnywhere script runs every day and I don't know how to schedule a task once a week :(
        ...
        print("Today is: {}".format(datetime.datetime.today().weekday()), file=sys.stderr)
